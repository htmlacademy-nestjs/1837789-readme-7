...
const validationSchema = Joi.object({
  ...
  rabbit: Joi.object({
    ...
    password: Joi.string().required(),
    port: Joi.number().port().default(Default.RabbitPort),
    user: Joi.string().required(),
  ...
  mail: Joi.object({
    host: Joi.string().valid().hostname().required(),
    port: Joi.number().port().default(Default.SmtpPort),
    user: Joi.string().required(),
    password: Joi.string().required(),
...
function getConfig(): NotifyConfig {
  const config: NotifyConfig = {
    ...
    rabbit: {
      ...
      port: parseInt(process.env.RABBIT_PORT ?? Default.RabbitPort.toString(), 10),
    ...
    mail: {
      ...
      port: parseInt(process.env.MAIL_SMTP_PORT ?? Default.SmtpPort.toString(), 10),
